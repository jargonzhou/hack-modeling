...
def get_flights():
  ...
  for flight in flights.keys():
    flights[flight] = sorted(
        flights[flight], key=lambda item: item["departure_time"])
  return flights
...
def cost_of_solution(solution):
  ...
  for i in range(len(solution)):
    ...
    if _get_minutes_in_day(return_flight["departure_time"]) < return_earliest_departure_time:
      return_earliest_departure_time = _get_minutes_in_day(
          return_flight["departure_time"])
  ...
  for i in range(len(solution)):
    ...
    out_flight = flights[(origin, destination)][solution[i][0]]
    return_flight = flights[(destination, origin)][solution[i][1]]
    total_wait_time += out_latest_arrival_time - \
        _get_minutes_in_day(out_flight["arrival_time"])
    total_wait_time += _get_minutes_in_day(
        return_flight["departure_time"]) - return_earliest_departure_time
    ...
    for i in range(len(_solution)):
      ...
      for j in range(len(out_deltas)):
        for k in range(len(return_deltas)):
          neighbours.append(
              _solution[0:i] + [(out_deltas[j], return_deltas[k])] + _solution[i + 1:])
  ...
  for i in range(iterations):
    ...
    while len(populations) < population_size:
      if random.random() < p_mutate:
        ...
        populations.append(cross_over(
            ranked_solutions[_index1], ranked_solutions[_index2]))

    if DEBUG:
      print("DEBUG>>> [{}] current best solution: {}".format(
          i, cost_function(populations[0])))
