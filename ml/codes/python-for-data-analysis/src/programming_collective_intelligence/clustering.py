...
from .tools.numeric_function import pearson
...
def hierarchical_clustering(matrix, labels, vector_distance_func=pearson):
  ...
  """
  # 最初的聚类是矩阵所有行向量
  nodes = [ClusterTreeNode(matrix[i], nid=i, data=labels[i])
  ...
  for _ in range(0, 1000):  # 迭代次数
    ...
    for k in range(n):
      for j in range(n):
        ...
        grad[k][0] += ((loc_2d[k][0] - loc_2d[j][0]) /
                       dist_2d[j][k]) * error_term
        grad[k][1] += ((loc_2d[k][1] - loc_2d[j][1]) /
                       dist_2d[j][k]) * error_term
        total_error += abs(error_term)
    print(total_error)

    # 结束条件
    if lasterror and lasterror < total_error:
      break
