...
# ---------------------------------------------------------------------------
from .tools.data_structure import CompareOperation, BaseDataSample
...
def show_data(datas):
  ...
  table = PrettyTable(['Referrer', 'Location', 'Read FAQ',
                      'Pages viewed', 'Service chosen'])
  table.align = "l"

  for data in datas:
    table.add_row([data.referrer(), data.location(), data.read_faq(),
                  data.pages_viewed(), data.get_target_value()])
  print(table)
  ...
  for attr in datas[0].get_attr_keys():
    ...
    for value in column_values:
      ...
      gain = current_score - p * \
          score_function(set1) - (1 - p) * score_function(set2)
      if DEBUG:
...
def display_tree(tree):
  ...
  def build_api_tree(tree, api_tree, parent=None, is_left=False):
    ...
    if parent is None:
      ...
      if is_left:
        parent = api_tree.create_node(
            tree_str, "L" + parent, parent=parent).identifier
      else:
        parent = api_tree.create_node(
            tree_str, "R" + parent, parent=parent).identifier
