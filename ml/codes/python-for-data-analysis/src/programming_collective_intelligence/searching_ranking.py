...
搜索和排序.
"""
from src.programming_collective_intelligence.import WORKING_DIR
import pprint

from .tools.database import connect_mysql
  ...
  for page in pages:
    ...
    for out_link in out_links:
      ...
      if out_url.startswith("#"):
        ...
        url_refs.append({"from": url, "to": out_base_url,
                        "text": out_link["content"]})
...
def show_data(data):
  ...
  for k, v in data["url_self_ref_dict"].items():
    ...
    table.add_row([k, len(data["url_texts_dict"][k]),
                  data["url_texts_dict"][k][:100], v])
  print(table)
  ...
  try:
    for url in urls:
      ...
      if url not in url_id_dict.keys():
        url_id_dict[url] = _save_url(
            connection, cursor, url, " ".join(url_texts_dict[url]))
      ...
      for i in range(len(words)):
        ...
        _save_url_word(connection, cursor,
                       url_id_dict[url], word_id_dict[word], i)
...
def _save_url(connection, cursor, url, content):
  ...
  :return:
  """
  sql_str = "INSERT INTO url(url, content) VALUES ('%s', '%s')" % (
      url, content)
  try:
...
def _save_link(connection, cursor, from_url_id, to_url_id):
  ...
  :return:
  """
  sql_str = "INSERT INTO link(from_url_id,to_url_id) VALUES ('%d', '%d')" % (
...
def _save_url_word(connection, cursor, url_id, word_id, location):
  ...
  :return:
  """
  sql_str = "INSERT INTO url_word(url_id, word_id, location) VALUES ('%d', '%d', '%d')" % (
...
def _save_link_word(connection, cursor, link_id, word_id):
  ...
  :return:
  """
  sql_str = "INSERT INTO link_word(link_id,word_id) VALUES ('%d', '%d')" % (
      link_id, word_id)
  try:
    cursor.execute(sql_str)
    connection.commit()
...
def search_word_location_on_url(query=None):
  ...
  for row in query_result:
    rows.append({"word": row[1], "url": row[3],
                "locations": sorted(map(int, row[4].split(",")))})
...
def search_rank_by_inbound_link(query=None):
  ...
  url_dict = dict([(url_id, url)
                  for (_, url_id, url) in word_urls])  # url_id => url
  url_inbound_count_dict = search_inbound_count(
      list(url_dict.keys()))  # url_id => count
  scores = dict([(url_dict[url_id], count)
                for (url_id, count) in url_inbound_count_dict.items()])
...
def calculate_page_rank(damping_factor=0.85, interations=20):
  ...
  for i in range(interations):
    ...
    page_ranks = dict(
        map(lambda item: (item[0], float(item[1]) / max_pr), page_ranks.items()))

  return sorted(
      [(pr, url_link_count_dict[url_id]["url"])
       for (url_id, pr) in page_ranks.items()],
      key=lambda item: item[0],
      reverse=True)
