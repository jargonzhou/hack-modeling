...
from src.programming_collective_intelligence.import STOP_WORDS
  ...
  def parse(self, response):
    ...
    for a_in_home in response.css("body main ol li a"):
      ...
      content = [word.lower() for word in
                 re.split(word_sep, a_in_home.css(
                     "::text").extract()[0].strip())
                 if word.lower() not in STOP_WORDS and word != '']
      url = a_in_home.attrib['href'].replace(
          START_URL, "").replace(BASE_PATH, "")
    ...
    for a_in_content in response.css("body main p a"):
      ...
      content = [word.lower() for word in
                 re.split(word_sep, a_in_content.css(
                     "::text").extract()[0].strip())
                 if word.lower() not in STOP_WORDS and word != '']
      url = a_in_content.attrib['href'].replace(
          START_URL, "").replace(BASE_PATH, "")
      out_links.append({"content": content, "url": url})
