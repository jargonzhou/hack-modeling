...
from src.programming_collective_intelligence.k_nearest_neighbors import wine_price, wine_set1, wine_set3, knn_estimate, knn_weighted_estimate, cumulative_graph, smooth_prob_graph
...
class TestKNN(unittest.TestCase):
  def test_wine_price(self):
    ...
    size = len(ratings)
    wine_prices = np.array([wine_price(rating, age)
                           for rating in ratings for age in ages]).reshape(size, size)
  ...
  def test_knn_weighted_estimate(self):
    ...
    vec1 = (45.0, 12.0)
    print(wine_price(*vec1), knn_estimate(data, vec1, k=10),
          knn_weighted_estimate(data, vec1, k=10))
    vec2 = (70.0, 12.0)
    print(wine_price(*vec2), knn_estimate(data, vec2, k=10),
          knn_weighted_estimate(data, vec2, k=10))
