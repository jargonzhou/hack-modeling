...
class TestTableRepresentation(unittest.TestCase):
  def test_load(self):
    ...
    exonerations = agate.Table.from_csv(
        _DATA_DIR + '/exonerations-20150828.csv', column_types=tester)
    print(exonerations)  # 表的描述

  def test_aggregate(self):
    exonerations = agate.Table.from_csv(
        _DATA_DIR + '/exonerations-20150828.csv')
  ...
  def test_charting(self):
    ...
    chart = counts.order_by('exonerated').line_chart(
        'exonerated', 'count', path="line_chart.svg")
    print(chart)

  def test_join(self):
    table1 = agate.Table.from_csv(_DATA_DIR + '/data-text.csv')
    print(table1)
    table2 = agate.Table.from_csv(_DATA_DIR + '/countries.csv')
    print(table2)
