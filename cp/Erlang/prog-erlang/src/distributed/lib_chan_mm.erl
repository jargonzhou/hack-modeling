...
%%% 中间人协议.
%%% 接收/发送: 
%%% {send, Term}/{chan, MM, Term}
%%% close/{chan_closed, MM}
%%% @end
%%%-------------------------------------------------------------------
...
close(Pid) ->
  ...
  Pid ! close.
controller(Pid, Pid1) ->
  ?DEBUG("controller", [Pid, Pid1]),
  Pid ! {setController, Pid1}.
set_trace(Pid, X) ->
  ?DEBUG("set_trace", [Pid, X]),
  Pid ! {trace, X}.
trace_with_tag(Pid, Tag) ->
  ?DEBUG("trace_with_tag", [Pid, Tag]),
  set_trace(Pid,
    {true, fun(Msg) -> io:format("MM:~p ~p~n", [Tag, Msg]) end}).


loop(Socket, Pid) ->
  ?DEBUG("loop", [Socket, Pid]),
  % 成为系统进程
  process_flag(trap_exit, true),
  F = fun(X) -> io:format("TRACE ~p~n", [X]) end,
  loop1(Socket, Pid, {true, F}). % false
